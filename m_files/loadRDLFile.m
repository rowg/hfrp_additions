...
%
%
% Version modified by Brian Emery to add ancillary data to the
% OtherMatrixVariables field ('ESPC','ETMP','MAXV','MINV','EDVC','ERTC')
...
% Add other matrix varibles to sub-structure
RADIAL.OtherMatrixVars = get_ancillary(vv,Data,RADIAL.OtherMatrixVars);

% Put warnings back in their initial state
%warning(ws)
end
%% ----------------------------------------------
function R = get_ancillary(vv,Data,R)
% GET ANCILLARY
% local function to get additional columns from Codar ctf format
% HF radar data.

% Brian Emery, March 2010

% ESPC ETMP MAXV MINV EDVC ERTC 
for m = {'ESPC','ETMP','MAXV','MINV','EDVC','ERTC'}; 
    m = m{:};
    ...
    if ~isempty(ii)
        ...
        % Put the data into the radial structure, naming the field after
        % the 4 char name codar uses:
        R.(m) = Data(:,ii);
      ...
      feval( 'warning', ['loadRDLFile:MISSING_ANCILLARY_DATA_COLUMN' ], ...
         'Ancillary Data column missing');
